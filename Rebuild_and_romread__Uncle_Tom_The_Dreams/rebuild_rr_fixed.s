; Dieses Programm ist urspruenglich entwickelt worden, um Screens,
...
; Translations, some comments and changes for better compatibility by Count Zero in 2012
; The original source can also be found along with this file as
; this was also changed to be assembled by dreamass.
; The extensions trashes the area from $a000 - $bfff and more for the most part, so just
; take it as example code.
;
; The text above reads:
;
; This program was originally developed to restore screens which
; are split by rasterinterrupts into e.g. charset- and bitmap-area
; into a single bitmap.
; It basically works but was never really finished. Any interested
; programmer may continue development on this!
; Keyboard commands can be found on the source code.
;
;(w) Uncle Tom/The Dreams
...
; these two routines are documented as "get col ram" and "set col ram" on the replay sources.
; using the jumps from the rom jump table makes this usable on many more replays (incl. cpx replay)
; the originally chosen addresses WILL fail on many cart roms
...
TX:              .text "1 - rebuild screen ",13,13
                 .text "2 - last edit",0
CHARANF:         .db 0
GOBEN:           .db 0
GUNTEN:          .db 0             
...
BSPEI:           .db 0,0
IOBUF:           .db 9
TABELLE00:       .db 0,0,0,0
TABELLE11:       .db $C0,$30,$0C,3
